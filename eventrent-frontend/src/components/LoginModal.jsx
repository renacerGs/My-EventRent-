...
// 👇👇👇 KOMPONEN OTP (DIGABUNG DI SINI BIAR GAMPANG) 👇👇👇
const OtpVerification = ({ email, onVerified, onCancel }) => {
  ...
  // 🚀 FUNGSI UTAMA BUAT NGE-TEMBAK API VERIFIKASI
  ...
  // 🚀 FITUR 1: AUTO-SUBMIT PAS NGETIK MANUAL
  const handleChange = (index, value) => {
    ...
    // Kalo kode udah 6 digit lengkap, sikat langsung (Auto Verify)
    const currentCode = newOtp.join('');
    if (currentCode.length === 6) {
      verifyOtpCode(currentCode);
    }
  };

  // 🚀 FITUR 2: BISA COPAS/PASTE LANGSUNG 6 DIGIT
  const handlePaste = (e) => {
    e.preventDefault();
    // Ambil text dari clipboard, buang yang bukan angka, ambil maksimal 6 digit
    ...
    if (pastedData) {
      ...
      // Pindahin kursor ke kotak terakhir yang berhasil diisi
      const focusIndex = Math.min(pastedData.length, 5);
      inputRefs.current[focusIndex].focus();

      // Kalo yang dipaste pas 6 digit, sikat langsung verifikasi!
    ...
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="w-full h-full flex flex-col justify-center items-center bg-white p-6 sm:p-8">
        ...
        <div className="flex justify-center gap-2 mb-6">
          {otp.map((digit, index) => (
            <input
              ...
              onPaste={handlePaste}
              disabled={isLoading}
              className="w-11 h-14 text-center text-xl font-black text-gray-800 bg-gray-50 border border-gray-200 rounded-xl focus:bg-white focus:border-[#FF6B35] focus:ring-2 focus:ring-[#FF6B35]/30 outline-none transition-all disabled:opacity-70"
            />
          ))}
        </div>
      ...
      <div className="mt-8 flex flex-col items-center gap-4 text-xs font-medium">
        <p className="text-gray-500">
          ...
          <button type="button" onClick={handleResend} disabled={resendCooldown > 0 || isLoading} className="text-[#FF6B35] font-black hover:underline disabled:text-gray-400 inline-flex items-center gap-1 uppercase tracking-widest">
            <RefreshCw className="w-3 h-3" /> {resendCooldown > 0 ? `Kirim ulang (${resendCooldown}s)` : 'Kirim Ulang'}
          </button>
        </p>
        <button onClick={onCancel} disabled={isLoading} className="text-gray-400 hover:text-gray-800 font-bold inline-flex items-center gap-1 uppercase tracking-widest mt-2 transition-colors disabled:opacity-50">
          <ArrowLeft className="w-3 h-3" /> Kembali Daftar
        </button>
      </div>
    </motion.div>
  );
...
export default function LoginModal({ isOpen, onClose, onLoginSuccess }) {
  ...
  // 🔥🔥🔥 EFEK PEMBERSIH STATE SAAT MODAL DITUTUP 🔥🔥🔥
  useEffect(() => {
    if (!isOpen) {
      setTimeout(() => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 sm:p-6 font-sans">
      ...
      >
        <button 
          onClick={onClose} 
          className={`absolute top-4 right-5 md:top-6 md:right-8 text-3xl md:text-2xl font-black transition-all z-[110] leading-none ${showOtpScreen ? 'text-gray-400 hover:text-gray-900' : 'text-white md:text-gray-400 hover:text-white/80 md:hover:text-gray-900'}`}
        >
          &times;
        </button>
