...
export default function EditPersonalEvent() {
    ...
    const fetchEventData = async () => {
      try {
        ...
        setEventDetails({
          ...
          profiles: details.profiles?.length > 0 ? details.profiles : [{ id: crypto.randomUUID(), fullName: '', nickName: '', role: 'Host', address: '', parentsInfo: '', photoUrl: null }],
  ...
  const handleGallerySelect = (e) => {
    ...
    if (galleryFiles.length + files.length > 5) return toast.error("Maximum of 5 gallery photos allowed!");
  ...
  const handleSaveCrop = async () => {
    ...
    } catch (e) { console.error(e); toast.error('Failed to crop image!'); }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!imageBase64) return toast.error("Main Cover/Poster must be uploaded!");
    if (!formData.eventStart || !formData.eventEnd) return toast.error("Please fill in the Start and End Dates!");
    ...
    try {
      ...
      if (response.ok) {
          ...
          toast.error("Failed to update invitation: " + (errorData.message || 'Server error'));
      }
    } catch (error) {
        console.error(error);
  ...
  if (isLoading) {
    return <div className="min-h-screen flex items-center justify-center bg-gray-50"><p className="font-bold text-purple-600 animate-pulse">Loading Event Data...</p></div>;
  }
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-20 font-sans relative transition-colors duration-500">
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white border border-gray-200 rounded-[24px] w-full max-w-2xl overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="p-6 bg-gray-50 flex flex-col sm:flex-row justify-between items-center gap-4">
              ...
              <button onClick={handleSaveCrop} className="w-full sm:w-auto px-8 py-3 bg-purple-600 text-white rounded-xl font-bold uppercase tracking-widest text-xs shadow-lg active:scale-95 hover:bg-purple-700 transition">✔ Crop & Save</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="pt-8 px-6 max-w-4xl mx-auto flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="text-sm font-bold flex items-center gap-2 text-purple-600 hover:text-purple-800 transition-colors">← Back</button>
      </div>

      <main className="max-w-4xl mx-auto px-6 py-6">
        <div className="text-center mb-10">
          <h1 className="text-4xl font-black uppercase tracking-tight text-gray-900">Edit Personal Event</h1>
          <p className="text-purple-600 mt-3 font-sans font-bold uppercase tracking-widest text-xs">Update the details of your personal event.</p>
        </div>

        <form onSubmit={handleSubmit}>
          
          <SectionAccordion title="★ CHOOSE EVENT THEME" isOpen={openSection === 'theme'} onToggle={() => toggleSection('theme')}>
            <p className="text-gray-500 text-sm mb-6 font-medium">Select the visual design that best suits your event.</p>
          ...
          <SectionAccordion title="1. Cover & Basic Information" isOpen={openSection === 'basic'} onToggle={() => toggleSection('basic')}>
            <div className="space-y-5">
              ...
              <div><label className={labelStyle}>Event Name</label><input type="text" name="title" placeholder="E.g., Budi's 20th Birthday / High School Reunion" value={formData.title} onChange={handleEventChange} className={inputStyle} required /></div>
              
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={labelStyle}>Start Date</label>
                  <CustomDatePicker theme="personal" value={formData.eventStart} onChange={(newDate) => handleEventChange({ name: 'eventStart', value: newDate })} placeholder="Select Start Date" />
                </div>
                <div>
                  <label className={labelStyle}>End Date</label>
                  <CustomDatePicker theme="personal" value={formData.eventEnd} onChange={(newDate) => handleEventChange({ name: 'eventEnd', value: newDate })} placeholder="Select End Date" />
                </div>
              </div>

              <div><label className={labelStyle}>Opening Message (Appears at the top of the invitation)</label><textarea name="openingMessage" placeholder="Hello friends, please join me at my event..." value={eventDetails.openingMessage} onChange={handleDetailsChange} rows="3" className={inputStyle} /></div>
              <div><label className={labelStyle}>Closing Message / Dresscode (Optional)</label><textarea name="closingMessage" placeholder="It would be great to have you there. Dress code: Casual All Black!" value={eventDetails.closingMessage} onChange={handleDetailsChange} rows="3" className={inputStyle} /></div>
            </div>
          </SectionAccordion>

          <SectionAccordion title="2. Host Profiles" isOpen={openSection === 'profiles'} onToggle={() => toggleSection('profiles')}>
            {eventDetails.profiles.map((prof, index) => (
               <div key={prof.id} className="p-6 border border-gray-100 bg-gray-50 rounded-xl mb-6 relative shadow-sm">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    {eventDetails.profiles.length > 1 && (<button type="button" onClick={() => removeProfile(prof.id)} className="text-red-500 text-xs font-bold uppercase hover:text-red-700">Remove</button>)}
                  </div>
                  <div className="flex flex-col md:flex-row gap-6">
                     <div className="w-full md:w-1/3">
                        ...
                        <label className="flex flex-col items-center justify-center w-full aspect-square border-2 border-dashed rounded-full cursor-pointer overflow-hidden border-purple-200 bg-white hover:bg-purple-50 transition-colors">
                          {prof.photoUrl ? (<img src={prof.photoUrl} alt="Profile" className="w-full h-full object-cover" />) : (<span className="text-2xl opacity-80">😎</span>)}
            ...
            <button type="button" onClick={addProfile} className="w-full py-3 border border-dashed border-purple-300 text-purple-600 rounded-xl font-bold uppercase text-xs tracking-widest hover:bg-purple-50 transition">+ Add Another Host</button>
          </SectionAccordion>
            ...
            {formData.sessions.map((session, sIndex) => (
              <div key={session.id || sIndex} className="p-6 border border-gray-100 bg-gray-50 rounded-xl mb-6 relative shadow-sm">
                <div className="flex justify-between items-center mb-4">
                  ...
                  {formData.sessions.length > 1 && (<button type="button" onClick={() => removeSession(sIndex)} className="text-red-500 text-xs font-bold uppercase hover:text-red-700">Remove Session</button>)}
                </div>
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      ...
                      <CustomDatePicker theme="personal" value={session.date} onChange={(newDate) => handleSessionChange(sIndex, 'date', newDate)} placeholder="Select Date" />
                    </div>
                    <div>
                  ...
                  <div><label className={labelStyle}>Maximum Guest Capacity</label><input type="text" value={session.stock} onChange={(e) => { const val = e.target.value; if (val === '' || /^\d+$/.test(val)) handleSessionChange(sIndex, 'stock', val); }} className={inputStyle} required /></div>
                  
                  <div className="mt-4 pt-4 border-t border-gray-200">
                    <p className="text-xs text-purple-600 font-bold mb-3 uppercase">Session Location</p>
                  ...
                  <div className="mt-6 pt-4 border-t border-gray-200">
                     <div className="mb-6"><p className="text-[10px] font-black uppercase tracking-widest mb-1 text-gray-500">RSVP / Registration Form For:</p><h2 className="text-xl font-bold text-gray-900">{session.name || `Session ${sIndex + 1}`}</h2></div>
                     
                     <div className="space-y-4 mb-8 select-none opacity-60">
                        <div><label className={labelStyle}>Full Name (Default)</label><input type="text" disabled className={`${inputStyle} bg-gray-100 text-gray-500 cursor-not-allowed`} value="Will be filled by guest upon RSVP" readOnly/></div>
                        <div><label className={labelStyle}>Email (Default)</label><input type="text" disabled className={`${inputStyle} bg-gray-100 text-gray-500 cursor-not-allowed`} value="Will be filled by guest upon RSVP" readOnly/></div>
                        <div><label className={labelStyle}>Number of People / Plus One (Automatic)</label><input type="text" disabled className={`${inputStyle} bg-gray-100 text-gray-500 cursor-not-allowed`} value="Will be filled by guest" readOnly/></div>
                        <div><label className={labelStyle}>Greeting / Short Message (Automatic)</label><textarea disabled className={`${inputStyle} bg-gray-100 text-gray-500 cursor-not-allowed`} rows="2" value="Guests can write their message here" readOnly/></div>
                     </div>
                        ...
                        <div key={q.id || qIndex} className="border rounded-xl p-5 mb-4 shadow-sm relative border-l-4 border-gray-200 border-l-purple-500 bg-white">
                          <div className="flex flex-col md:flex-row gap-4 mb-3 w-full">
                            <div className="flex-1 min-w-0">
                              <input type="text" placeholder="Type your additional question (E.g., Favorite song request)" value={q.text} onChange={(e) => handleQuestionChange(sIndex, qIndex, 'text', e.target.value)} className={`${inputStyle} w-full`} required />
                            </div>
                            ...
                            <div className="pl-4 border-l-2 mt-4 space-y-2 border-purple-200">
                              ...
                              {q.options?.map((opt, optIndex) => (
                                <div key={optIndex} className="flex items-center gap-3">
                                  ...
                                  <input type="text" placeholder={`Option ${optIndex + 1}`} value={opt} onChange={(e) => updateQuestionOption(sIndex, qIndex, optIndex, e.target.value)} className={`${inputStyle} py-2 flex-1`} required />
                                  {q.options.length > 1 && (
                              ...
                              <button type="button" onClick={() => addQuestionOption(sIndex, qIndex)} className="text-xs font-bold mt-2 px-3 py-1.5 rounded-lg transition-colors text-purple-600 hover:bg-purple-50">+ Add Option</button>
                            </div>
                          )}
                          <div className="flex justify-end items-center gap-4 mt-6 pt-4 border-t border-gray-100">
                            <button type="button" onClick={() => removeQuestion(sIndex, qIndex)} className="text-xs font-bold uppercase tracking-widest transition text-gray-400 hover:text-red-500">Remove Field</button>
                     ...
                     <button type="button" onClick={() => addQuestion(sIndex)} className="text-sm font-bold flex items-center gap-2 mt-4 transition text-purple-600 hover:text-purple-800"><span className="text-xl">⊕</span> + Add Extra Question</button>
                  </div>
                </div>
              </div>
            ))}
            <button type="button" onClick={addSession} className="w-full py-3 border border-dashed border-purple-300 text-purple-600 rounded-xl font-bold uppercase text-xs tracking-widest hover:bg-purple-50 transition">+ Add Another Session</button>
          </SectionAccordion>

          <SectionAccordion title={`4. Photo Gallery (${galleryFiles.length}/5)`} isOpen={openSection === 'gallery'} onToggle={() => toggleSection('gallery')}>
             <div className="space-y-4">
                <p className="text-sm text-gray-500">Choose up to 5 photos, and add a caption (optional).</p>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                   {galleryFiles.map((gf, i) => (
                      <div key={i} className="group flex flex-col gap-2">
                         <div className="relative aspect-[3/4] rounded-xl overflow-hidden border border-gray-200 shadow-sm">
                            ...
                            <button type="button" onClick={() => removeGalleryImage(i)} className="absolute inset-0 z-10 bg-black/60 flex items-center justify-center text-white font-bold opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">Remove</button>
                         </div>
                         <input type="text" placeholder="Write a caption for this photo..." value={gf.caption} onChange={(e) => handleGalleryCaptionChange(i, e.target.value)} className="w-full px-3 py-2 text-xs rounded-lg border border-gray-300 focus:outline-none focus:border-purple-500 bg-white" />
                      </div>
                   ))}
                   {galleryFiles.length < 5 && (
                      <label className="flex flex-col items-center justify-center w-full aspect-[3/4] border-2 border-dashed rounded-xl cursor-pointer transition-all border-purple-200 bg-white hover:bg-purple-50">
                         <span className="text-2xl text-purple-400 opacity-80 mb-2">+</span><span className="text-[10px] font-bold uppercase tracking-widest text-purple-600">Select Photo</span>
          ...
          <SectionAccordion title="5. Digital Gifts (Optional)" isOpen={openSection === 'gifts'} onToggle={() => toggleSection('gifts')}>
            <p className="text-sm text-gray-500 mb-6">Guests can send gifts directly via bank transfer or e-wallet.</p>
            {eventDetails.digitalGifts.map((gift, index) => (
              <div key={gift.id} className="p-5 border border-gray-200 bg-gray-50 rounded-xl mb-4 flex flex-col md:flex-row gap-4 items-end shadow-sm">
                ...
                <div className="w-full md:w-1/3"><label className={labelStyle}>Account / Phone Number</label><input type="text" value={gift.accountNumber} onChange={(e) => handleGiftChange(gift.id, 'accountNumber', e.target.value)} className={inputStyle} /></div>
                <div className="w-full md:w-1/3"><label className={labelStyle}>Account Holder Name</label><input type="text" value={gift.accountName} onChange={(e) => handleGiftChange(gift.id, 'accountName', e.target.value)} className={inputStyle} /></div>
            ...
            <button type="button" onClick={addGift} className="w-full py-3 border border-dashed border-purple-300 text-purple-600 rounded-xl font-bold uppercase text-xs tracking-widest hover:bg-purple-50 transition">+ Add Another Account</button>
          </SectionAccordion>

          <div className="flex justify-between items-center pt-6 mt-10 border-t border-gray-200">
            <button type="button" onClick={() => navigate(-1)} className="px-8 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition text-gray-500 hover:text-gray-800">Cancel</button>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl transform transition-all text-center">
            ...
            <p className="text-gray-500 text-sm font-medium mb-2">
              Personal invitation updated successfully.
            </p>
          </div>
        </div>
      )}
