...
export default function MyTickets() {
  ...
  const openQR = (trx, globalIndex) => {
    const attendeeName = (trx.attendee_data && trx.attendee_data[0]?.name) 
      ? trx.attendee_data[0].name 
      : `Peserta ${globalIndex + 1}`;
    
    setSelectedQR({ 
      ...trx, 
      // PERBAIKAN: FORMAT QR disamakan pakai JSON biar Scanner gak pusing!
  ...
  const OrderCardGroup = ({ orderGroup, isPast }) => {
    ...
    return (
      <div className={`bg-white rounded-[32px] shadow-sm border border-gray-200 overflow-hidden transition-all hover:shadow-md mb-6 ${isPast ? 'opacity-70 grayscale-[30%]' : ''}`}>
        ...
        <AnimatePresence>
          {isExpanded && (
            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t-2 border-dashed border-gray-200 bg-gray-50/50">
              <div className="p-6 md:p-8 space-y-8">
                
                {/* PERBAIKAN: Mengelompokkan tampilan tiket berdasarkan sesi/kelas */}
                {(() => {
                  const sessionGroups = {};
                    ...
                    <div key={sIndex} className="relative">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pl-4">
                        {sg.tickets.map((trx, tIndex) => {
                          const attendeeName = trx.attendee_data?.[0]?.name || `Peserta ${tIndex + 1}`;
                          const isScanned = trx.is_scanned; 
                          
                          return (
                            <div key={trx.ticket_id} className={`bg-white border rounded-2xl p-4 flex items-center justify-between shadow-sm transition-colors ${isScanned ? 'border-gray-200 opacity-60' : 'border-gray-200 hover:border-[#FF6B35]'}`}>
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] pt-10 px-4 md:px-8 pb-20 font-sans">
      <AnimatePresence>
        {showQR && selectedQR && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="bg-white w-full max-w-sm rounded-[32px] shadow-2xl overflow-hidden">
              ...
              <div className="p-8 flex flex-col items-center -mt-10 relative z-10">
                ...
                <div className="text-center mb-8 w-full">
                  <span className="bg-orange-50 text-[#FF6B35] border border-orange-100 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest mb-4 inline-block">E-Ticket {selectedQR.attendeeNum}</span>
                  <h3 className="text-2xl font-black text-gray-900 uppercase tracking-widest mb-3">{selectedQR.attendeeName}</h3>
                ...
                <button onClick={() => window.print()} className="w-full py-4 bg-[#FF6B35] text-white rounded-2xl font-bold hover:bg-[#E85526] transition-all uppercase tracking-widest text-xs shadow-xl shadow-orange-100 active:scale-95">Download E-Ticket</button>
              </div>
            </motion.div>
          </div>
        )}
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-12">
          <h2 className="text-lg font-bold text-gray-900 mb-5 flex items-center gap-3"><span className="w-2.5 h-2.5 rounded-full bg-green-500 shadow-[0_0_8px_#22c55e] animate-pulse"></span>Active Tickets</h2>
          <div>
            {activeEvents.length > 0 ? (
              ...
              <div className="text-center py-16 text-gray-400 bg-white rounded-[32px] border border-gray-100 shadow-sm">
                <div className="text-4xl mb-3">🎟️</div><p className="font-bold">Belum ada tiket aktif.</p>
              </div>
            )}
          </div>
        </div>
        {pastEvents.length > 0 && (
          <div>
            <h2 className="text-lg font-bold text-gray-400 mb-5 flex items-center gap-3"><span className="w-2.5 h-2.5 rounded-full bg-gray-300"></span>History (Past Events)</h2>
            <div>{pastEvents.map(orderGroup => <OrderCardGroup key={orderGroup.order_id} orderGroup={orderGroup} isPast={true} />)}</div>
          </div>
        )}
      </div>
    </div>
