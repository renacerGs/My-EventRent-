...
export default function CreatePersonalEvent() {
  ...
  // 🔥 AUTO-SAVE DRAFT: Tarik data awal formData dari localStorage
  const [formData, setFormData] = useState(() => {
    const savedDraft = localStorage.getItem('draft_create_personal_event');
    if (savedDraft) {
      try {
  ...
  // 🔥 AUTO-SAVE DRAFT: Tarik data awal eventDetails dari localStorage
  ...
  // 🔥 AUTO-SAVE DRAFT: Tarik gambar cover dari localStorage
  ...
  useEffect(() => {
    try {
      ...
      localStorage.setItem('draft_create_personal_event', JSON.stringify(draftData));
    } catch (error) {
      ...
      localStorage.setItem('draft_create_personal_event', JSON.stringify(textOnlyDraft));
    }
  ...
  const addProfile = () => {
    setEventDetails(prev => ({
      ...prev,
      profiles: [...prev.profiles, { id: crypto.randomUUID(), fullName: '', nickName: '', role: 'Co-Host', address: '', parentsInfo: '', photoUrl: null }]
    }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
          ...formData,
          userId: user.id, // Pastikan userId terkirim
          img: coverUrl,
      ...
      if (response.ok) {
          ...
          localStorage.removeItem('draft_create_personal_event');
          ...
          <SectionAccordion title="★ CHOOSE INVITATION THEME" isOpen={openSection === 'theme'} onToggle={() => toggleSection('theme')}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div 
                  ...
                  className={`cursor-pointer rounded-3xl border-4 p-4 text-center transition-all ${eventDetails.templateType === 'ThemeBirthday' ? 'border-pink-400 bg-pink-50' : 'border-gray-200 bg-white hover:border-pink-200'}`}
               >
                  <div className="h-32 bg-gradient-to-br from-pink-300 via-blue-200 to-yellow-200 rounded-2xl mb-4 flex items-center justify-center shadow-inner">
               ...
               <div 
                  onClick={() => setEventDetails({ ...eventDetails, templateType: 'ThemePartyNight' })}
                  className={`cursor-pointer rounded-3xl border-4 p-4 text-center transition-all ${eventDetails.templateType === 'ThemePartyNight' ? 'border-purple-500 bg-purple-50' : 'border-gray-200 bg-white hover:border-purple-200'}`}
               >
               ...
               <div 
                  onClick={() => setEventDetails({ ...eventDetails, templateType: 'ThemeCasual' })}
                  className={`cursor-pointer rounded-3xl border-4 p-4 text-center transition-all ${eventDetails.templateType === 'ThemeCasual' ? 'border-emerald-500 bg-emerald-50' : 'border-gray-200 bg-white hover:border-emerald-200'}`}
               >
                  <div className="h-32 bg-gradient-to-br from-emerald-100 to-teal-50 rounded-2xl mb-4 flex items-center justify-center shadow-inner">
