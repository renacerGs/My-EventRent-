        ...
        {user ? (
          <>
            ...
            <div className="relative" ref={profileRef}>
              ...
              {/* 1. TOMBOL PROFIL BULAT (Trigger) */}
              <button 
                onClick={() => setIsDropdownOpen(!isDropdownOpen)}
                className={`w-[42px] h-[42px] rounded-full p-[2px] transition-all duration-300 focus:outline-none shadow-sm
                  ${isDropdownOpen ? 'ring-2 ring-[#FF6B35] scale-105' : 'hover:scale-105 border border-gray-200'}`}
              >
                <img 
              ...
              {/* 2. ISI DROPDOWN (CARD STYLE) */}
              {isDropdownOpen && (
                <div className="absolute right-0 top-full mt-3 w-[280px] bg-white rounded-[24px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] border border-gray-100 transform origin-top-right transition-all animate-fadeIn z-[60] overflow-hidden">
                  
                  {/* HEADER: INFO USER (Warna Light Orange) */}
                  <div className="bg-orange-50/80 p-5 flex items-center justify-between">
                    <div className="overflow-hidden pr-2">
                      <p className="font-bold text-gray-900 truncate text-[15px]">{user.name}</p>
                      <p className="text-[11px] text-gray-500 truncate font-medium">{user.email}</p>
                    </div>
                    {/* Avatar Kecil di Kanan (Sesuai Referensi) */}
                    <div className="w-10 h-10 rounded-full border-2 border-white shadow-sm overflow-hidden flex-shrink-0">
                       <img src={user.picture || "https://api.dicebear.com/7.x/avataaars/svg?seed=Felix"} alt="User" className="w-full h-full object-cover" />
                    </div>
                  </div>
                  ...
                  <div className="p-2 flex flex-col gap-1">
                    ...
                    <Link 
                      ...
                      className="flex items-center gap-3 px-4 py-3 text-sm font-semibold text-gray-600 hover:bg-gray-50 hover:text-gray-900 rounded-xl transition-all"
                    >
          ...
          /* TAMPILAN BELUM LOGIN (Login Button) */
          <button 
            onClick={onOpenLogin}
