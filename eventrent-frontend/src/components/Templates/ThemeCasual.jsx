...
/* ═══════════════════════════════════════════════════════════════
   MAIN COMPONENT — Theme Professional
   ═══════════════════════════════════════════════════════════════ */
export default function ThemeProfessional({ eventData, guestName, isOpen, onOpen, navigate, id }) {
  ...
  // Mencegah error jika profile kosong
  if (!Array.isArray(profilesList) || profilesList.length === 0) {
    profilesList = [{
      fullName: eventData?.title || "Organisasi/Perusahaan",
      role: "Penyelenggara",
  ...
  // 🔥 PERBAIKAN GALERI: Tarik array foto dan caption dengan aman
  const gallery = Array.isArray(details.galleryImages) ? details.galleryImages : (Array.isArray(details.gallery_images) ? details.gallery_images : []);
  const galleryCaptions = Array.isArray(details.galleryCaptions) && details.galleryCaptions.length > 0 ? details.galleryCaptions : ["Suasana Acara", "Sesi Diskusi", "Dokumentasi", "Networking", "Presentasi Materi"];
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}>
            ...
            <section className="py-24 px-6 bg-corporate-alt">
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-5xl mx-auto text-center">
                <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-12 uppercase tracking-widest">Informasi Penyelenggara</h2>

                {/* 🔥 PERBAIKAN: Menggunakan FLEXBOX agar kalau 1 orang otomatis di tengah, kalau banyak berjejer rapi 🔥 */}
                <div className="flex flex-wrap justify-center items-stretch gap-8">
                  {profilesList.map((prof, idx) => {
                    const name = prof.fullName || prof.full_name || prof.name || "Nama Tidak Tersedia";
                    const nickname = prof.nickName || prof.nick_name || "";
                    const role = prof.role || "Penyelenggara";
                    ...
                    return (
                      <motion.div 
                        ...
                        className="card-professional p-8 sm:p-10 flex flex-col items-center w-full sm:w-[340px]"
                      >
                        {/* 🔥 FOTO: aspect-square, object-cover, shrink-0 memastikan foto kotak dan membulat sempurna di tengah 🔥 */}
                        <div className="w-32 h-32 mb-6 rounded-full overflow-hidden border-4 border-white shadow-lg bg-white flex items-center justify-center shrink-0 aspect-square">
                          <img src={photo} alt={name} className="w-full h-full object-cover" />
                        </div>

                        <p className="text-xs uppercase tracking-[0.2em] text-blue-600 mb-2 font-bold">{role}</p>
                        <h3 className="text-xl font-black text-slate-900 mb-1 text-center">{name}</h3>
                        
                        {/* Menampilkan Nickname jika ada */}
                        {nickname && (
                          <p className="text-sm text-slate-500 italic font-medium mb-3">"{nickname}"</p>
                        )}
                ...
                <div className="space-y-6 text-left">
                  {sessions.map((session, idx) => (
                    <motion.div key={idx} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: 0.1 * idx, duration: 0.5 }} className="card-professional p-8">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between border-b border-slate-100 pb-4 mb-6">
                        <div className="max-w-xl">
                          <h3 className="text-xl font-black text-slate-800 uppercase tracking-wide">{session.name}</h3>
                          {/* 🔥 Menambahkan Deskripsi Agenda 🔥 */}
                        ...
                        <span className="mt-4 sm:mt-0 inline-block px-3 py-1 bg-blue-50 text-blue-700 text-[10px] font-bold uppercase tracking-widest rounded-full border border-blue-100 w-max shrink-0">
                          Sesi {idx + 1}
                        </span>
                      </div>
              ...
              <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-2xl mx-auto">
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 p-8 rounded-2xl inline-block">
                  ...
                  <p className="text-2xl font-black text-white uppercase tracking-wide">
                    {profilesList[0]?.fullName || profilesList[0]?.full_name || eventData?.title || "Penyelenggara"}
                  </p>
                </div>
              </motion.div>
            </section>
