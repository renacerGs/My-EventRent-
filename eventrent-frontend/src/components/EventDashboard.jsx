...
export default function EventDashboard() {
  ...
  return (
    <div className="bg-[#F8F9FA] min-h-screen font-sans pb-20 pt-8 text-left relative">
      ...
      {/* 1. POPUP SUCCESS / ERROR */}
      {popup.show && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4 transition-all">
          <div className="bg-white rounded-[32px] p-8 max-w-sm w-full shadow-2xl transform text-center relative overflow-hidden">
                        ...
                        <div className="flex items-center justify-end gap-4">
                          {!a.is_scanned && (
                            <button 
                              ...
                              className="text-[10px] font-bold text-[#27AE60] uppercase tracking-wider hover:underline transition-all">
                              Check-In
                            </button>
                          )}
                          <button onClick={() => toggleExpand(a.ticket_id)} className="text-[10px] font-bold text-[#FF6B35] uppercase tracking-wider hover:underline transition-all">
                        ...
                        <td colSpan="6" className="px-8 py-6 border-l-4 border-[#FF6B35]">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {a.attendee_data?.map((p, i) => (
                              <div key={i} className="bg-white p-5 rounded-2xl border border-orange-100 shadow-sm relative overflow-hidden flex flex-col justify-between">
                                {/* Highlight MATCH pencarian */}
                                {searchQuery && p.name.toLowerCase().includes(searchQuery.toLowerCase()) && (
                                  <div className="absolute top-0 right-0 bg-[#FF6B35] text-white text-[8px] font-black uppercase px-2 py-1 rounded-bl-lg shadow-sm">
                                    MATCH
                                  </div>
                                )}
                                
                                <div>
                                  <p className="text-[10px] font-black text-[#FF6B35] uppercase mb-1 tracking-widest">Attendee {i+1}</p>
                                  <p className="font-bold text-gray-900 text-lg mb-1">{p.name}</p>
                                  <p className="text-xs font-semibold text-gray-500 mb-4">{p.email}</p>
                                </div>
                                
                                {/* --- JAWABAN CUSTOM DENGAN TOOLTIP KEREN --- */}
                                {p.customAnswers && p.customAnswers.length > 0 && (
                                  <div className="mt-2 pt-3 border-t border-gray-100 flex flex-wrap gap-2">
                                    {p.customAnswers.map((ans, idx) => (
                                      <div 
                                        key={idx} 
                                        className="cursor-help bg-gray-50 border border-gray-200 px-3 py-1.5 rounded-lg flex items-center gap-2 hover:bg-orange-50 hover:border-orange-200 transition-colors group relative"
                                      >
                                        ...
                                        {/* TOOLTIP ALA TAILWIND */}
                                        <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-gray-900 text-white text-[10px] font-bold rounded-xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10 whitespace-nowrap shadow-xl">
                                          {ans.question}
                  ...
                  <tr>
                    <td colSpan="6" className="px-8 py-12 text-center text-gray-400 font-bold text-sm">
                      <div className="flex flex-col items-center justify-center">
                        ...
                        Tidak ada data peserta yang cocok dengan <span className="text-gray-600 mt-1">"{searchQuery}"</span>
                      </div>
                    </td>
                  </tr>
                </tbody>
