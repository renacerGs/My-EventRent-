              ...
              <table className="hidden md:table w-full text-left border-collapse min-w-[900px]">
                <thead>
                  <tr className="bg-gray-50/50 border-b border-gray-100 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    <th className="px-8 py-5">ID</th><th className="px-8 py-5">{isWed ? 'Nama Tamu' : 'Buyer Account'}</th><th className="px-8 py-5 max-w-[200px]">Session(s)</th><th className="px-8 py-5 text-center">{isWed ? 'Pax' : 'Qty'}</th><th className="px-8 py-5">Status</th><th className="px-8 py-5 text-right">Action</th>
                  </tr>
                </thead>
                    ...
                    <tbody key={`desktop-${order.order_id}`} className="border-b border-gray-50 last:border-none">
                      <tr className="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td className="px-8 py-5">
                          {order.is_attending_all === false ? (<span className="px-3 py-1 bg-red-50 text-red-500 rounded-full text-[10px] font-black uppercase tracking-widest border border-red-100">Tidak Hadir</span>) : order.scanned_qty === order.total_qty ? (<span className="px-3 py-1 bg-[#E7F9F1] text-[#27AE60] rounded-full text-[10px] font-black uppercase tracking-widest border border-green-100">All Checked-In</span>) : order.scanned_qty > 0 ? (<span className="px-3 py-1 bg-orange-50 text-[#FF6B35] rounded-full text-[10px] font-black uppercase tracking-widest border border-orange-100">Partial ({order.scanned_qty}/{order.total_qty})</span>) : (<span className="px-3 py-1 bg-gray-50 text-gray-400 rounded-full text-[10px] font-bold uppercase tracking-widest">Belum Hadir</span>)}
                        </td>
                        <td className="px-8 py-5 text-right"><button onClick={() => toggleExpand(order.order_id)} className="text-[10px] font-bold text-gray-500 hover:text-[#FF6B35] uppercase tracking-wider underline">{expandedRow === order.order_id ? 'Tutup Detail' : 'Lihat Detail'}</button></td>
                      </tr>
                                  ...
                                  <div key={t.ticket_id} className={`bg-white p-5 rounded-2xl border ${isWed ? 'border-[#D4AF37]/30 shadow-md' : 'border-orange-100 shadow-sm'} relative overflow-hidden flex flex-col justify-between`}>
                                    ...
                                    <div>
                                      <div className="flex justify-between items-start mb-2">
                                        <div>
                                          <p className={`text-[10px] font-black uppercase mb-1 ${isWed ? 'text-[#D4AF37]' : 'text-[#FF6B35]'}`}>{isWed ? 'Data Tamu' : `Tiket #${t.ticket_id}`}</p>
                                    ...
                                    {!t.is_scanned && t.is_attending !== false && (
                                      <button onClick={() => initiateManualCheckIn(t.ticket_id)} className={`w-full mt-4 text-[10px] font-black text-white py-2.5 rounded-xl uppercase tracking-widest shadow-sm transition-colors ${isWed ? 'bg-slate-900 hover:bg-black' : 'bg-[#FF6B35] hover:bg-orange-600'}`}>Manual Check-In</button>
                                    )}
                                  </div>
                                )
                              })}
                  ...
                  currentItems.map((order) => (
                    <div key={`mobile-${order.order_id}`} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm">
                      ...
                      <div className="flex justify-between items-center mt-3">
                        <div>
                          {order.is_attending_all === false ? (<span className="px-2.5 py-1 bg-red-50 text-red-500 rounded-full text-[9px] font-black uppercase tracking-widest border border-red-100">Tidak Hadir</span>) : order.scanned_qty === order.total_qty ? (<span className="px-2.5 py-1 bg-[#E7F9F1] text-[#27AE60] rounded-full text-[9px] font-black uppercase tracking-widest border border-green-100">All Hadir</span>) : order.scanned_qty > 0 ? (<span className="px-2.5 py-1 bg-orange-50 text-[#FF6B35] rounded-full text-[9px] font-black uppercase tracking-widest border border-orange-100">Sebagian ({order.scanned_qty}/{order.total_qty})</span>) : (<span className="px-2.5 py-1 bg-gray-50 text-gray-400 rounded-full text-[9px] font-bold uppercase tracking-widest">Belum Hadir</span>)}
                        </div>
                        <button onClick={() => toggleExpand(order.order_id)} className="text-[10px] font-black text-[#FF6B35] uppercase tracking-wider flex items-center gap-1 bg-orange-50 px-3 py-1.5 rounded-lg active:scale-95 transition-all">{expandedRow === order.order_id ? 'Tutup' : 'Lihat Data'}</button>
                      </div>
                      {expandedRow === order.order_id && (
                        <div className="mt-4 pt-4 border-t-2 border-dashed border-orange-100 flex flex-col gap-3">
                          {order.tickets.map((t, idx) => (
                            <div key={t.ticket_id} className={`p-4 rounded-xl border shadow-sm ${t.is_attending === false ? 'bg-red-50/20 border-red-100' : t.is_scanned ? 'bg-green-50/30 border-green-100' : 'bg-white border-gray-200'}`}>
                              <div className="flex justify-between items-start mb-2">
                                <div><p className={`text-[9px] font-black uppercase mb-1 ${isWed ? 'text-[#D4AF37]' : 'text-gray-500'}`}>{isWed ? 'Data Tamu' : `TIKET #${t.ticket_id.toString().slice(-5)}`}</p><span className="bg-gray-100 text-gray-600 text-[8px] px-2 py-0.5 rounded uppercase font-bold tracking-wider inline-block">{t.session_name}</span></div>
                                {t.is_attending === false ? (<span className="text-[8px] font-black bg-red-100 text-red-600 px-2 py-0.5 rounded-full uppercase mt-1">Tidak Hadir</span>) : t.is_scanned ? (<span className="text-[8px] font-black bg-green-100 text-green-600 px-2 py-0.5 rounded-full uppercase mt-1">Hadir</span>) : null}
                              </div>
