  ...
  return (
    <div className="bg-slate-950 min-h-screen pb-20 font-sans relative transition-colors duration-500">
      ...
      <div className="pt-8 px-6 max-w-4xl mx-auto flex items-center gap-4">
        <button onClick={() => navigate(-1)} className="text-sm font-bold flex items-center gap-2 text-[#D4AF37] hover:opacity-80 transition-colors">← Kembali</button>
      </div>
      ...
      <main className="max-w-4xl mx-auto px-6 py-6">
        ...
        <form onSubmit={handleSubmit}>
          ...
          <SectionAccordion title="1. Pilih Tema Undangan" isOpen={openSection === 'template'} onToggle={() => toggleSection('template')}>
          ...
          <SectionAccordion title="2. Cover & Informasi Dasar" isOpen={openSection === 'basic'} onToggle={() => toggleSection('basic')}>
             <div className="space-y-5">
              ...
              <div><label className={labelStyle}>Judul Undangan</label><input type="text" name="title" value={formData.title} onChange={handleEventChange} className={inputStyle} required /></div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className={labelStyle}>Tanggal Mulai Acara</label><input type="date" name="eventStart" value={formData.eventStart} onChange={handleEventChange} className={inputStyle} required /></div>
                <div><label className={labelStyle}>Tanggal Selesai</label><input type="date" name="eventEnd" value={formData.eventEnd} onChange={handleEventChange} className={inputStyle} required /></div>
              </div>
              <div><label className={labelStyle}>Kata Pengantar (Doa)</label><textarea name="openingMessage" value={eventDetails.openingMessage} onChange={handleDetailsChange} rows="3" className={inputStyle} /></div>
              <div><label className={labelStyle}>Kata Penutup</label><textarea name="closingMessage" value={eventDetails.closingMessage} onChange={handleDetailsChange} rows="3" className={inputStyle} /></div>
              <div><label className={labelStyle}>Quotes Romantis</label><textarea name="quote" value={eventDetails.quote} onChange={handleDetailsChange} rows="2" className={inputStyle} /></div>
            </div>
          </SectionAccordion>
             ...
             {eventDetails.profiles.map((prof, index) => (
               <div key={prof.id} className="p-6 border border-slate-700 bg-slate-800/30 rounded-xl mb-6 relative">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    {eventDetails.profiles.length > 1 && <button type="button" onClick={() => removeProfile(prof.id)} className="text-red-400 text-xs font-bold uppercase">Hapus</button>}
                  </div>
          ...
          <SectionAccordion title={`5. Galeri Foto (${galleryFiles.length}/5)`} isOpen={openSection === 'gallery'} onToggle={() => toggleSection('gallery')}>
             <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                {galleryFiles.map((gf, i) => (
                    <div key={i} className="relative aspect-[3/4] rounded-xl overflow-hidden group border border-slate-700">
                      <img src={gf.preview} alt="Galeri" className="w-full h-full object-cover" />
                      <button type="button" onClick={() => removeGalleryImage(i)} className="absolute inset-0 bg-black/60 flex items-center justify-center text-white font-bold opacity-0 group-hover:opacity-100">Hapus</button>
                    </div>
                ))}
          ...
          <SectionAccordion title="6. Amplop Digital (Opsional)" isOpen={openSection === 'gifts'} onToggle={() => toggleSection('gifts')}>
             {eventDetails.digitalGifts.map((gift) => (
              <div key={gift.id} className="p-5 border border-slate-700 bg-slate-800/30 rounded-xl mb-4 flex flex-col md:flex-row gap-4 items-end">
                ...
                <div className="w-full md:w-1/3"><label className={labelStyle}>No. Rekening</label><input type="text" value={gift.accountNumber} onChange={(e) => handleGiftChange(gift.id, 'accountNumber', e.target.value)} className={inputStyle} /></div>
            ...
            <button type="button" onClick={addGift} className="w-full py-3 border border-dashed border-[#D4AF37] text-[#D4AF37] rounded-xl font-bold text-xs hover:bg-[#D4AF37]/10">+ Tambah Rekening Lain</button>
          </SectionAccordion>

          <div className="flex justify-between items-center pt-6 mt-10 border-t border-slate-800">
            <button type="button" onClick={() => navigate(-1)} className="px-8 py-4 rounded-xl font-bold uppercase tracking-widest text-xs transition text-gray-500 hover:text-white">Batal</button>
