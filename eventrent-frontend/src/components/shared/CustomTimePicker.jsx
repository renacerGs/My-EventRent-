...
export default function CustomTimePicker({ value, onChange, placeholder = "Pilih Jam", theme = "public" }) {
  ...
  return (
    <div className="relative w-full">
      {/* 👇 FIX: Ganti dari div biasa jadi button type="button" */}
      <button 
        type="button" 
        onClick={(e) => { e.preventDefault(); setIsOpen(true); }} 
        className={`w-full rounded-xl px-4 py-3 text-sm transition-all bg-white text-gray-900 border border-gray-300 ${colors.borderHover} cursor-pointer flex items-center justify-between shadow-sm`}
      >
        ...
        {isOpen && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 10 }} className={`${colors.bg} p-2 rounded-[2rem] w-full max-w-[280px] shadow-2xl`}>
              <div className="bg-white rounded-[1.5rem] p-9 w-full">
                ...
                <div className="flex justify-center items-center gap-4 mb-6">
                  ...
                  <div className="w-20 h-[160px] overflow-y-auto snap-y snap-mandatory border border-gray-200 rounded-2xl bg-gray-50 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    {hours.map(h => (
                      <button type="button" key={h} onClick={(e) => { e.preventDefault(); setHour(h); }} className={`w-full h-12 flex items-center justify-center snap-center cursor-pointer font-bold text-xl transition-colors ${h === hour ? `${colors.bg} text-white rounded-xl mx-1 my-1 shadow-md` : `text-gray-400 ${colors.hoverText}`}`}>
                        {h}
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="w-20 h-[160px] overflow-y-auto snap-y snap-mandatory border border-gray-200 rounded-2xl bg-gray-50 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
                    {minutes.map(m => (
                      <button type="button" key={m} onClick={(e) => { e.preventDefault(); setMinute(m); }} className={`w-full h-12 flex items-center justify-center snap-center cursor-pointer font-bold text-xl transition-colors ${m === minute ? `${colors.bg} text-white rounded-xl mx-1 my-1 shadow-md` : `text-gray-400 ${colors.hoverText}`}`}>
                        {m}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  <button type="button" onClick={(e) => { e.preventDefault(); setIsOpen(false); }} className="flex-1 text-gray-500 hover:bg-gray-50 py-3.5 rounded-[1rem] font-bold uppercase tracking-widest text-[10px] transition-all active:scale-95 border border-gray-100">Batal</button>
                </div>
              </div>
            </motion.div>
          </div>
