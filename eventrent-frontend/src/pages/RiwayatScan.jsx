  ...
  return (
    <div className="bg-[#0f172a] min-h-screen font-sans pb-20 relative text-left">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-8 md:pt-12 relative z-10">
        ...
        {/* HEADER */}
        <div className="flex items-center gap-3 md:gap-4 mb-8 relative z-20">
          <button 
            type="button" 
            onClick={() => navigate(-1)} 
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 md:w-12 md:h-12 bg-blue-500/20 text-blue-500 rounded-2xl flex items-center justify-center border border-blue-500/30 shrink-0 hidden md:flex">
            ...
            <div>
              <h1 className="text-xl md:text-3xl font-black text-white uppercase tracking-tight">Scan History</h1>
              <p className="text-[10px] md:text-sm font-medium text-slate-400 mt-1">Guest check-in activity log by {user?.name || 'Agent'}.</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 p-4 md:p-6 rounded-[24px] border border-slate-700/50 mb-8 backdrop-blur-sm shadow-xl relative z-20">
          <div className="flex flex-col md:flex-row gap-4 items-end">
            <div className="flex flex-col gap-1 w-full">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">From Date</span>
              <div className="w-full">
                <CustomDatePicker value={startDate} onChange={setStartDate} placeholder="Select Start Date" theme="dark" />
              </div>
            </div>
            <div className="flex flex-col gap-1 w-full">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">To Date</span>
              <div className="w-full">
                <CustomDatePicker value={endDate} onChange={setEndDate} placeholder="Select End Date" theme="dark" />
              </div>
            </div>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-slate-800/30 border border-slate-700/50 rounded-[32px] py-20 text-center relative z-10">
             ...
             <p className="font-black text-white text-lg tracking-tight mb-1">No History</p>
             <p className="text-xs font-medium text-slate-400">You haven't scanned any tickets in this date range.</p>
          </div>
        ) : !activeEventTracker ? (
          <div className="relative z-10">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Event List ({eventListSummary.length})</p>
              ...
              {eventListSummary.map((evt, idx) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between border-t border-slate-700/50 pt-4 mt-auto">
                    <div>
                      <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-0.5">Your Total Scans</p>
                      <p className="font-black text-emerald-400 text-lg">{evt.totalScans} <span className="text-xs font-bold text-slate-400">Tickets</span></p>
                    </div>
          ...
          <div className="relative z-10 animate-in fade-in slide-in-from-bottom-4 duration-300">
            ...
            <div className="bg-slate-800/50 p-4 rounded-[20px] border border-slate-700/50 mb-6 flex flex-col md:flex-row gap-4 relative z-20">
              <div className="relative w-full md:flex-1 group h-[46px]">
                <input 
                  type="text" 
                  placeholder="Search Ticket ID or Guest Name..." 
                  value={searchQuery}
            ...
            {detailedFilteredHistory.length === 0 ? (
               <div className="bg-slate-800/30 border border-slate-700/50 rounded-3xl py-12 text-center text-slate-400 text-sm font-bold">No attendee data matches your search.</div>
            ) : (
              <>
                  ...
                  <table className="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                      <tr className="bg-slate-900/80 border-b border-slate-700 text-[10px] font-black text-slate-400 uppercase tracking-widest">
                        <th className="px-8 py-5 text-blue-400">Ticket & Guest Details</th>
                        ...
                        <tr key={idx} className="border-b border-slate-700/50 hover:bg-slate-700/40 transition-colors">
                          ...
                          <td className="px-8 py-5 text-right">
                            <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-emerald-500/10 text-emerald-400 rounded-xl text-[10px] font-black uppercase tracking-widest border border-emerald-500/20 shadow-sm shadow-emerald-500/5">
                              <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7"></path></svg> Success
                            </span>
                          </td>
                        </tr>
                      ))}
                  ...
                  {detailedFilteredHistory.map((log, idx) => (
                    <div key={idx} className="bg-slate-800/60 border border-slate-700/50 rounded-[24px] p-5 shadow-lg relative overflow-hidden">
                      ...
                      <div className="border-t border-slate-700/50 pt-4 text-right">
                         <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Scan Time</p>
