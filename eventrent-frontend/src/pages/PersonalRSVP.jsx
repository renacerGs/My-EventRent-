...
export default function PersonalRSVP() {
  ...
  if (isLoading) return <div className="min-h-screen bg-gray-50 flex items-center justify-center"><div className="animate-spin w-10 h-10 border-4 border-slate-200 border-t-slate-600 rounded-full"></div></div>;
  if (!event) return <div className="text-center mt-20 text-gray-500 font-bold">Event not found.</div>;

  const inputStyle = `w-full bg-white text-gray-900 border border-gray-200 rounded-xl px-5 py-4 focus:border-slate-500 focus:ring-1 focus:ring-slate-500 outline-none transition text-sm placeholder:text-gray-400`;

  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans text-gray-800">
      
      <div className="bg-white border-b border-gray-100 px-4 py-4 flex items-center sticky top-0 z-50 shadow-sm">
        <button onClick={() => navigate(-1)} className="text-gray-400 p-2 bg-gray-50 rounded-full hover:text-slate-600 hover:bg-slate-50 transition">
        ...
        <div className="w-full max-w-2xl bg-white border border-gray-100 rounded-[32px] shadow-xl p-6 sm:p-10 relative overflow-hidden">
          ...
          {isSuccess ? (
            <div className="text-center py-12 relative z-10">
              <CheckCircle2 className="w-24 h-24 text-slate-500 mx-auto mb-6" />
              ...
              <p className="text-slate-600 text-xs uppercase tracking-widest font-bold animate-pulse">
            ...
            <form onSubmit={handleSubmit} className="space-y-8 relative z-10">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button 
                    ...
                    className={`py-4 rounded-xl text-sm font-bold transition-all border-2 ${formData.isAttending === true ? 'border-slate-500 bg-slate-50 text-slate-700 shadow-sm' : 'border-gray-200 bg-gray-50 text-gray-400 hover:border-slate-200 hover:bg-white'}`}
                  >
                    Yes, I'm In!
                  </button>
                  <button 
              ...
              {formData.isAttending === true && (
                <div className="space-y-8 p-6 bg-slate-50/50 border border-slate-100 rounded-2xl">
                  
                  <div>
                    <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-widest mb-3">Which Sessions Will You Attend? <span className="text-red-500">*</span></label>
                    <div className="space-y-3">
                      {event.sessions?.map(s => (
                        <label 
                          key={s.id} 
                          onClick={() => toggleSession(s.id)} 
                          className={`flex items-center gap-4 p-4 rounded-xl border-2 cursor-pointer transition-all bg-white ${selectedSessions.includes(s.id) ? 'border-slate-500 shadow-md' : 'border-gray-100 hover:border-slate-200'}`}
                        >
                          <div className={`w-5 h-5 rounded flex items-center justify-center border-2 transition-colors ${selectedSessions.includes(s.id) ? 'bg-slate-500 border-slate-500' : 'border-gray-300'}`}>
                             {selectedSessions.includes(s.id) && <CheckCircle2 className="w-3 h-3 text-white" strokeWidth={4} />}
                          </div>
                          <div className="flex-1">
                            <p className={`font-black text-sm ${selectedSessions.includes(s.id) ? 'text-slate-700' : 'text-gray-700'}`}>{s.name}</p>
                  ...
                  <div>
                    <label className="block text-[11px] font-bold text-slate-600 uppercase tracking-widest mb-3">How Many People? (Pax) <span className="text-red-500">*</span></label>
                    <div className="grid grid-cols-2 gap-4">
                      <button 
                        type="button"
                        onClick={() => setFormData({...formData, pax: 1})}
                        className={`py-5 flex flex-col items-center justify-center gap-2 rounded-xl transition-all border-2 bg-white ${formData.pax === 1 ? 'border-slate-500 text-slate-700 shadow-md' : 'border-gray-100 text-gray-400 hover:border-slate-200'}`}
                      >
                      ...
                      <button 
                        ...
                        className={`py-5 flex flex-col items-center justify-center gap-2 rounded-xl transition-all border-2 bg-white ${formData.pax === 2 ? 'border-slate-500 text-slate-700 shadow-md' : 'border-gray-100 text-gray-400 hover:border-slate-200'}`}
                      >
                  ...
                  {activeQuestions.length > 0 && (
                    <div className="pt-6 border-t border-slate-100 space-y-6">
                      <h3 className="text-[11px] font-black text-slate-600 uppercase tracking-widest bg-slate-100/50 inline-block px-3 py-1 rounded-md">Additional Questions</h3>
                            ...
                            <div className="space-y-3 mt-2">
                              {q.options?.map((opt, i) => (
                                <label key={i} className="flex items-center gap-3 cursor-pointer group">
                                  <input type="checkbox" value={opt} onChange={e => handleCustomChange(q.id, opt, 'Checkbox', e.target.checked)} className="w-5 h-5 text-slate-600 border-gray-300 rounded focus:ring-slate-500" />
                                  <span className="text-sm font-medium text-gray-700 group-hover:text-slate-700 transition-colors">{opt}</span>
                                </label>
                              ))}
                            </div>
                          )}
              ...
              <div className="pt-4 mt-8">
                <button 
                  ...
                  className="w-full py-4 bg-slate-600 text-white font-black uppercase tracking-widest rounded-xl hover:bg-slate-700 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-slate-200 text-sm"
                >
